...
//		query= session.createQuery("from Account a where a.balance > 30000");
		...
		/*in this since lazy join is there in inner for loop entire queries are re-executed
		 * 
		...
		query= session.createQuery("from User u left outer join fetch u.accounts");
		List<User> obj_arr= query.list();
		for (User objects : obj_arr) {
			//System.out.println(objects[0]+" ------ "+objects[1]);
			//User u= (User) objects[0];
			System.out.println(objects.getFullname());
			for (Account a: objects.getAccounts()) {
				System.out.println(a.getBalance());
